...
export function renderServiceCard(service: Service): string {
  return `<div class="svc-card card-stagger">
<div class="svc-head"><span class="svc-name">${service.name}</span><span class="svc-price">${service.price}</span></div>
<div class="svc-desc">${service.description}</div>
<a class="svc-link" href="${service.url}">${service.linkText} &rarr;</a>
</div>`;
}
...
export function renderProjectCard(project: Project): string {
  ...
  return `<div class="project-card card-stagger">
...
export function renderCollaboratorCard(collab: Collaborator): string {
  return `<div class="collab-card card-stagger"><div class="collab-name">${collab.name}</div><div class="collab-role">${collab.role}</div></div>`;
}
...
<div class="tl-label">${entry.date}</div>
<div class="tl-text">${entry.event}</div>
</div>`,
    )
    .join("\n\n");
...
export function renderWalletCard(wallet: WalletInfo): string {
  return `<div class="wallet-card">
<span class="wallet-label">${wallet.label}</span>
<span class="wallet-addr"><a href="${wallet.explorerUrl}">${wallet.address}</a><button class="copy-btn" data-addr="${wallet.address}">copy</button></span>
</div>`;
}
...
export function renderHTML(
  ...
  const collabCards = collaborators.map(renderCollaboratorCard).join("\n");
  const timelineHtml = renderTimeline(timeline);
  const walletCards = wallets.map(renderWalletCard).join("\n");
